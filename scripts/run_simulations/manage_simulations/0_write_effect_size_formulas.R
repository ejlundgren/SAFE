...
fwrite(effect_formulas, "remote_mirrors/all_effect_sizes_august_2025/data/effect_size_formulas.csv", na = "NA")
fwrite(effect_formulas, "remote_mirrors/SAFE_B_sims_july_2025/data/effect_size_formulas.csv", na = "NA")

unique(effect_formulas$cloud_filtering_rules)
unique(effect_formulas[name == "lnCVR", .(sim_family, cloud_filtering_rules)])

effect_formulas <- fread("data/effect_size_formulas.csv")
effect_formulas[is.na(cloud_filtering_rules)]

effect_formulas[name == "lnOR"]$sim_family
effect_formulas[name == "lnRR"]$sim_family

effect_formulas[name == "lnOR"]$cloud_filtering_rules
...
p1 + (1/2) * p2
(n_AA/(n_AA + n_Aa + n_aa)) + (1/2)*(n_Aa/(n_AA + n_Aa + n_aa))
# Hmmmm.


# Equation 30 seems transcribale without the earlier equations, which I don't fully understand.

# log((p2) / (2*sqrt(p1*p3)))
# p1: (n_AA / (n_AA + n_Aa + n_aa))
# p2: (n_Aa / (n_AA + n_Aa + n_aa))
# p3: (n_aa / (n_AA + n_Aa + n_aa))
log((n_Aa/(n_AA + n_Aa + n_aa)) / (2*sqrt((n_AA/(n_AA + n_Aa + n_aa)) * (n_aa/(n_AA + n_Aa + n_aa)))))
# Should be -1.2747


# Variance
(1/n) * ((1/p2) + (1-p2)/(4*p1*p3))
(1/(n_AA + n_Aa + n_aa)) * ((1/(n_Aa / (n_AA + n_Aa + n_aa))) + (1-(n_Aa / (n_AA + n_Aa + n_aa)))/(4*(n_AA / (n_AA + n_Aa + n_aa))*(n_aa / (n_AA + n_Aa + n_aa))))
# should be 0.05125
