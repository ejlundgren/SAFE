...
# Remove files that need to be redone:
files <- list.files("remote_mirrors/final_simulations/outputs/",
                    full.names = T)
# file.remove(files[grepl("lnRR", files)])
# file.remove(files[grepl("lnOR", files)])
# file.remove(files[grepl("HWE", files)])

# >>> Copy relevant data --------------------------------------------------

formulas <- fread("data/effect_size_formulas.csv")
fwrite(formulas, "remote_mirrors/final_simulations/data/effect_size_formulas.csv")
formulas[name == "lnHWE_A"]$cloud_filtering_rules # must be NA
formulas[name == "lnOR"]$sim_family # must be 4_binomial
formulas[name == "lnCVR"]$cloud_filtering_rules # must be NA

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -------------------------------------
# Load scenarios ------------------------------------------------------

# First step is to use GLOBUS to copy the remote files.
# The remote_meta_simulations.R script loads working_scenarios.Rds
# We can change that locally (and the shell scripts) and then update remote with GLOBUS.
...
if(run){
  ...
  # Save a small subset:
  unique(scenarios$checkpoint_path)
  working_scenarios <- copy(scenarios)
  saveRDS(working_scenarios, "data/working_scenarios.Rds")
