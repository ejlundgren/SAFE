...
The simulations take a long time to run on a personal computer. We have therefore uploaded all simulation results as .Rds files, though we show exactly how we ran each simulation.

To run the code interactively (instead of just reading this lovely tutorial), do the following:

1.  Clone the github repo: https://github.com/itchyshin/SAFE_bootstrap.git
2.  Download the raw simulation from **XXXXX** and place one directory level above the github project repo.
3.  Run the code chunks in the online_tutorial.qmd while in the R project session.
...
In these Monte Carlo simulation, we were interested in bias, or the difference between the 'true' estimands (i.e., sampling variance and effect sizes) and the estimates (of sampling variance and effect sizes) produced from various methods and applied to the simulated data.
...
Load the final simulation for lnRoM here instead:
...
point.bias <- res[, .(plugin_1st = (mean(sim_y_plugin_1st) - unique(true_y_plugin_1st)),
                      plugin_2nd = (mean(sim_y_plugin_2nd) - unique(true_y_plugin_1st)),
                      safe = (mean(yi_safe) - unique(true_y_plugin_1st))),
...
ggplot(data = point.bias.long[sample_size1 == sample_size2, ],
  ...
  scale_color_discrete("Estimator",
                       labels = c("plugin_1st" = "1st order plugin (definition formula)",
                                  "plugin_2nd" = "2nd order plugin (bias-corrected formula)",
...
Interestingly, there is no way to know the 'true' estimand for sampling variance. **WHICH I DON'T FULLY UNDERSTAND**. To calculate bias for our sampling variance estimates, we actually calculate the variance in point estimates from each estimator and use each of these as the estimand. For 3 estimators, we'll thus end up with 9 calculations of bias.

Since we're interested in relative bias (not absolute bias as with the point estimates because we're not interested in whether estimates are more negative or positive), we'll calculate bias but divide by the estimand and multiply by 100.

```{r}
#| eval: true

# First, calculate variance in point estimates as our 'estimand'
res[, variance_estimand_1st_order := var(sim_y_plugin_1st), 
    by = .(scenario_id)]
res[, variance_estimand_2nd_order := var(sim_y_plugin_2nd), 
    by = .(scenario_id)]
res[, variance_estimand_SAFE := var(yi_safe), 
    by = .(scenario_id)]
...
# Now let's calculate relative bias:

var.bias <- res[, .(SAFE_estimate.1st_order_estimand = relative_bias(vi_safe, variance_estimand_1st_order),
                    SAFE_estimate.2nd_order_estimand = relative_bias(vi_safe, variance_estimand_2nd_order),
                    SAFE_estimate.SAFE_estimand = relative_bias(vi_safe, variance_estimand_SAFE),
                    plugin_1st_estimate.1st_order_estimand = relative_bias(sim_v_plugin_1st, variance_estimand_1st_order),
                    plugin_1st_estimate.2nd_order_estimand = relative_bias(sim_v_plugin_1st, variance_estimand_2nd_order),
                    plugin_1st_estimate.SAFE_estimand = relative_bias(sim_v_plugin_1st, variance_estimand_SAFE),
                    plugin_2nd_estimate.1st_order_estimand = relative_bias(sim_v_plugin_2nd, variance_estimand_1st_order),
                    plugin_2nd_estimate.2nd_order_estimand = relative_bias(sim_v_plugin_2nd, variance_estimand_2nd_order),
                    plugin_2nd_estimate.SAFE_estimand = relative_bias(sim_v_plugin_2nd, variance_estimand_SAFE)),
    by = .(scenario_id, sample_size1, sample_size2)] |> unique()
...
# Split the 'variable' to make more sense.
var.bias.long[, c("Estimator", "Estimand") := tstrsplit(variable, ".", fixed = TRUE)]
var.bias.long[, Estimator := gsub("_estimate", "", Estimator)]
var.bias.long[, Estimator := gsub("_", " ", Estimator)]
var.bias.long[, Estimand := gsub("_estimand", "", Estimand)]
var.bias.long[, Estimand := gsub("_", " ", Estimand)]
...
We see here that SAFE is more biased than the other estimators but not by much.
