...
if(DEBUG){
  ...
  library("MASS")
  library("tmvtnorm")
  # source("scripts/active_scripts/1_universal_SAFE.R")
...
eff_size <- function(..., #' [This is where the input variables are passed in.]
  ...
  safe_out <- parallel::mclapply(1:max(lengths(input_vars)), function(k){
    if(verbose) cat("SAFE:", magenta(k, "/", max(lengths(input_vars)), "\r"))
...
parameter_cloud <- function(formulas,
  ...
  if(!all(is.na(formulas$lower_filter))){
    ...
    var_guide <- data.table::data.table(variable = lower[[1]] |> trimws(),
      ...
      merge(data.table(mean=means |> as.numeric(), variable = names(means)),
            by = "variable")
    
    var_guide
    
  }else if(all(is.na(formulas$lower_filter)) &
           unique(formulas$sim_family) %in% c("1_normal", "4_multivariate_normal",
  ...
  return(cat(red("Unexpected error 1: sim_family not supported")))
}
