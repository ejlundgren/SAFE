...
3.  As long as your data consists of independent groups, you can draw your bootstrapped variables in separate `rnorm` (or `rbinom`) calls as we did above. If your data is bounded (e.g., by 0 as with lnRoM and lnCVR), you'll need to draw values within a `while` loop, excluding values that are not compatible with your effect size. If there is dependency between groups (i.e., paired designs), you'll need to use slightly more complicated methods with multivariate draws and sigma-matrices as described in the manuscript (Nakagawa et al. 2025). Note that multivariate draws (i.e., with `MASS::mvrnorm()`) take variance instead of SD. In the next section, we'll share some sample code of how to use SAFE with these alternative bootstrapping functions.

After making those decisions, simply follow the 4 steps to 'SAFEty'. 🙃

## Beyond `rnorm`

While the function `rnorm` can be used for a wide variety of effect sizes, some problems may occur. For example, some effect sizes account for paired designs (e.g., before-after) and thus require multivariate bootstrapping, where the two distributions are drawn based on a covariance matrix. Others, such as the log-transformed ratio of means (also known as the response ratio), are bounded between 0 and infinity. Finally, some effect sizes, such as the log odds ratio and log risk ratio, require discrete binomial distributions. Let's work through a few examples of how to draw SAFE bootstraps for these effect sizes.
...
Some of the statistics required in effect sizes are bounded, such as between 0 and positive infinity. lnRoM and lnCVR are such effect sizes and can't be calculated if a mean is ≤0. To do a SAFE bootstrap calculation of an effect size like lnRoM, especially if the mean of a group is near 0, one can nest your draw inside a `while` loop and filter out any simulated parameters that are ≤0. To avoid getting 'stuck' in a while loop quagmire, we recommend having an exit condition based on elapsed time. To demonstrate:

```{r}
#| label: bounded_values
#| echo: true
#| attr-source: "style='font-size: 0.8em;'"
#| attr-output: "style='font-size: 0.7em'"

total_boots <- 1e6      # Total hyperparameter draw length of SAFE bootstrap
cloud_rows <- 0         # Current number of drawn hyperparameters
duration <- 0           # Duration
s <- Sys.time()         # Start time
cloud_list <- list()    # List of draws
index <- 1              # Index variable

while(cloud_rows < total_boots & duration < 120){
  cloud_list[[index]] <- MASS::mvrnorm(n = total_boots,
                 mu = c(x1, x2),
                 Sigma = sigma_matrix) |> 
    as.data.frame() |> 
    setDT()
  
  cloud_list[[index]] <- cloud_list[[index]][V1 > 0 & V2 > 0]
  duration <- as.numeric(Sys.time() - s)
  cloud_rows <- sapply(cloud_list, nrow) |> sum()
  index <- index + 1
}

cloud <- rbindlist(cloud_list)
setnames(cloud, c("V1", "V2"), c("x1", "x2"))

# Now filter to the bootlength (drop the extra parameters)
cloud <- cloud[1:total_boots, ]

```

One can then use the same code as above to calculate bias-corrected point and sampling variance estimates on the `cloud` object.
...
To conduct SAFE on a binomial distribution like these, one uses 'rbinom()' to draw 'a' and 'c' based on the probability of 'a' and 'c' (e.g., $a / n_{\text{1}}$ and $c / n_{\text{2}}$). Note that values of 0 for any of the variables ('a'-'d') create `NA` values. For that reason, we apply a continuity correction to the SAFE binomial draws by adding 0.5 to 'a'-'d' if any of these variables are equal to 0.
...
a_cloud <- rbinom(1e6, n1, pr_a) |> as.numeric()
c_cloud <- rbinom(1e6, n2, pr_c) |> as.numeric()

b_cloud <- n1 - a_cloud
d_cloud <- n2 - c_cloud

cloud <- data.table(a = a_cloud,
                    b = b_cloud,
                    c = c_cloud,
                    d = d_cloud)

# Apply continuity correction:
cloud[(a == 0 | b == 0 | c == 0 | d == 0), 
        `:=` (a = a + 0.5,
              b = b + 0.5,
              c = c + 0.5,
              d = d + 0.5)]

cloud_trans = log((cloud$a * cloud$d) / (cloud$b * cloud$c))
...
Now let's look at Monte Carlo simulation results for a variety of effect sizes. We'll show point-estimate bias, the relative bias of the sampling variance, and *coverage*. Coverage describes whether the CIs of the estimate include the 'true' point estimand.
...
And coverage. Here we're plotting the proportion of Monte Carlo simulations where the 'true' estimand was inside the 95% confidence intervals of the estimates.
```{r}
...
Now let's run this in a foreach loop. Note that in this simulation, we're evaluating SAFE estimates using only the 'true' values, instead of drawing simulated data.
...
# Paired design Monte Carlo {#sec-paired-design-monte-carlo}
...
paired_dat <- readRDS("run_simulations/remote_mirrors/revision_paired_simulations/summaries/paired_scenarios_bias.Rds")
sub_dat <- paired_dat[effect_type == "SMD_paired", ]
...
facet_labs <- as_labeller(c("first_estimand" = "Cohen's d\nestimand",
                            "second_estimand" = "Hedges' g\nestimand",
                            "safe_estimand" = "SAFE\nestimand",
                            "0" = "r = 0",
                            "0.5" = "r = 0.5",
                            "0.8" = "r = 0.8"))
...
p.point <- ggplot(data = sub_dat[measure %in% c("bias") &
  ...
  xlab("Sample size")+
  ggtitle("SMD", subtitle = "Point estimate bias")+
  scale_color_manual(name = "Estimator", 
                     values = pal,
                     labels = labs)+
...
p.variance <- ggplot(data = sub_dat[measure %in% c("rel_bias"), ], 
  ...
  scale_color_manual(name = "Estimator", 
                     values = pal,
                     labels = c(first_estimate = "Cohen's d", 
                                second_estimate = "Hedges' g", 
                                safe_estimate = "SAFE"))+
  ggtitle("SMD", subtitle = "Variance relative bias")+
...
sub_dat <- paired_dat[effect_type == "lnRoM_paired" & !is.na(value), ]
...
p.point <- ggplot(data = sub_dat[measure %in% c("bias") &
  ...
  xlab("Sample size")+
  ggtitle("lnRoM", subtitle = "Point estimate bias")+
  scale_color_manual(name = "Estimator", 
                     values = pal,
                     labels = labs,
                     drop = TRUE)+
...
p.variance <- ggplot(data = sub_dat[measure %in% c("rel_bias"), ], 
  ...
  scale_color_manual(name = "Estimator", 
                     ...
                     labels = c(first_estimate = "1st order plugin", 
                                plugin_2nd = "2nd order plugin", 
                                safe = "SAFE"))+
  ggtitle("lnRoM", subtitle = "Variance relative bias")+
...
First, the bias of the point estimates:
```{r}
#| label: lnCVR_paired_point_bias
#| eval: true
#| echo: false
#| attr-source: "style='font-size: 0.8em;'"
#| attr-output: "style='font-size: 0.7em'"

sub_dat <- paired_dat[effect_type == "lnCVR_paired" & !is.na(value), ]
sub_dat <- droplevels(sub_dat)
...
facet_labs <- as_labeller(c("first_estimand" = "1st order\nestimand",
                            "second_estimand" = "2nd order\nestimand",
                            "safe_estimand" = "SAFE\nestimand",
                            "0" = "r = 0",
                            "0.5" = "r = 0.5",
                            "0.8" = "r = 0.8"))
...
p.point <- ggplot(data = sub_dat[measure %in% c("bias") &
  ...
  scale_color_manual(name = "Estimator", 
                     ...
                     labels = labs,
                     drop = TRUE)+
  ggtitle("lnCVR", subtitle = "Point estimate bias")+
  coord_cartesian(ylim = c(-0.1, 0.1))+
  theme_SAFE+
  theme(legend.position = "bottom") 
p.point
...
p.variance <- ggplot(data = sub_dat[measure %in% c("rel_bias"), ], 
  ...
  geom_path(lwd = 1)+
  geom_point(size = pt_size)+
  ylab("Relative bias of variance estimate")+
  xlab("Sample size")+
  scale_color_manual(name = "Estimator", 
                     values = pal,
                     labels = c("first_estimate" = "1st order plugin", 
                                "second_estimate" = "2nd order plugin", 
                                "safe_estimate" = "SAFE"))+
  ggtitle("lnCVR", subtitle = "Variance relative bias")+
...
# Prevalence of zero exclusions
One of the challenges with SAFE is that some effect sizes are limited to positive values, requiring us to filter out draws. This of course changes the realized distribution of the data, which could bias SAFE estimates if this occurs frequently. 

Here is the average number of excluded values (in this case values < 0) from the above simulations on lnRoM and lnCVR (which are bounded to between 0 and positive Infinity):

```{r}
#| label: prevalence_of_zeroes
#| eval: true
#| echo: false
library("knitr")

zeroes <- readRDS("run_simulations/remote_mirrors/revision_paired_simulations/summaries/exclusion_summaries.Rds")

zeroes <- zeroes[effect_type %in% c("lnCVR_paired", "lnRoM_paired"), 
                 .(mean_number_excluded = mean(mean_excluded)),
                 by = .(effect_type, r)]
setnames(zeroes, 
         c("effect_type", "mean_number_excluded"),
         c("Effect type", "Mean number excluded"))
kable(zeroes)
