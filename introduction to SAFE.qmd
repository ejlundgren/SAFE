...
Now we will run 100,000 simulations to demonstrate how this method works. Simulation results for the other effect sizes are reported below (@Monte-Carlo-results).
...
We've conducted full Monte Carlo simulations for a variety of effect sizes, which are below (@Monte-Carlo-results). However, first, we'll demonstrate how a meta-analyst might use SAFE to calculate point estimates and sampling variances for a custom effect size.
