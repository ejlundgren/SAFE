...
Our speed example above is very simple. How would one use SAFE to calculate bias-corrected point estimates and variance for a more scientifically interesting and challenging effect size? Let's take a look at the temperature coefficient Q~10~. Q~10~ is a widely used measure of temperature sensitivity of chemical reactions or biological processes that compares the rate of some process between 2 temperature treatment groups. The formula for Q~10~ is:

$$
Q_{\text10} = (R_{\text2} / R_{\text{1}})^{10/(T_{\text2}-T_{\text1})}
$$ {#eq-Q10}

Where `R` is the rate of some biological process for treatment groups 1 and 2 and `T` is the temperatures of groups 1 and 2.

If one wanted to conduct a meta-analysis on studies reporting Q~10~ data, along with sample size and standard deviation for each mean `R`, how would you calculate sampling variance? One could derive sampling variance using the delta method but this exceeds many people's mathematical prowess and it's not clear how one would evaluate whether the derived sampling variance is accurate or not. 
...
First, let's encapsulate the Q~10~ plugin formula in a function and then calculate Q~10~ point estimates for each row of our fake data.
...
Now, let's use SAFE to calculate a bias corrected point estimate and sampling variance. To do this, we will draw hyperparameter clouds for R1 and R2 based on the standard error (`SD/sqrt(N)`), transform these clouds with our Q10 function, calculate sampling variance of this transformed cloud, and then calculate our bias-corrected point estimates. 
...
Sampling variance for Q~10~! This sampling variance can then be used be in a meta-analytic model, like usual:
...
Before we go any further, and show how well SAFE works compared to plugin methods, let's look at a function we wrote to calculate effect sizes both with plugin estimators and with SAFE. This function can be sourced into the local environment. Let's load the function and then calculate effect sizes, with SAFE, for several examples from the main text. The function is located in the github repo at scripts/SAFE_function.R. 
