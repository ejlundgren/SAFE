...
Our speed example above is very simple. How would one use SAFE to calculate bias-corrected point estimates and variance for a more scientifically interesting and challenging effect size? Let's take a look at the temperature coefficient Q~10~. Q~10~ is a widely used measure of temperature sensitivity for chemical reactions or biological processes. The formula for Q~10~ is:

$$
Q_{\text10} = (R_{\text2} / R_{\text{1}})^{10/(T_{\text2}-T_{\text1})}
$$ {#eq:Q10}
Where `R` is the rate of some biological process for treatment groups 1 and 2 and `T` is the temperature of the experimental treatment for groups 1 and 2.

If one wanted to conduct a meta-analysis on studies reporting Q~10~ data, along with sample size and standard deviation for each `R`, how would you calculate sampling variance? One could derive sampling variance using the delta method but this exceeds most people's mathematical prowess and it's not clear how one would evaluate whether the derived sampling variance works or not. 

For example, @havird2020, conducted an influential meta-analysis on passive versus active thermal acclimation among a diversity of aquatic and terrestrial taxa but did not derive sampling variance and thus used unweighted meta-analytic models—thus ignoring uncertainty and sampling effort within each study. Let's see if we can use SAFE to calculate sampling variance for Q10 data. We will use a fake dataset, representing what a meta-analyst might collect from the literature.
...
First, let's encapsulate the Q~10~ formula in a function and then calculate Q10 point estimates for each row of our fake data.
...
We now have calculated sampling variance for a novel effect size. This sampling variance can be used be in a meta-analytic model as follows:
...
Pretty pretty neat, right?

If you want to use SAFE for other custom effect sizes, you'd follow a similar process:

1. Determine which variables need to be drawn in your effect size formula. Note that with Q~10~, the two variables that needed to be drawn were `R1` and `R2`, as these were associated with SD and N in the reporting studies. `T1` and `T2` were constants and thus did not need to be drawn for SAFE calculations.
...
3. As long as your data consists of independent groups, you can draw your variables of interest in separate `rnorm` (or `rbinom`) calls. However, if there is dependency between groups, you'll need to use slightly more complicated methods with multivariate draws and sigma-matrices as described in the manuscript (Nakagawa et al. 2025).

After making those decisions, simply following the 4 steps to 'SAFEty'. 🙃
