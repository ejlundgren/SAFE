...
# Compare to transformed cloud of hyperparameters:
...
- `...`: Named vectors (numeric) of the variables required by the selected effect size formula (see `eff_size()`). All supplied vectors must have identical lengths. The required variable names depend on `effect_type` and must match the `vars_required` field for that effect in `data/effect_size_formulas.csv` (see `eff_size()`). If a paired design is requested and \code{r} is not supplied, it defaults to 0.5; for non-paired designs, missing \code{r} defaults to 0.
- `effect_type`: Character (scalar). Name of the effect size to compute, matching a \code{name} value in `data/effect_size_formulas.csv` (see `eff_size()`). If \code{NULL}, the function returns a lookup table of available effect sizes and their required variables and exits.
- `SAFE`: Logical. If \code{TRUE} (default), perform SAFE bias correction using parametric bootstrap draws specified by \code{SAFE_boots}. If \code{FALSE}, only the plugin effect(s) are returned.
- `SAFE_boots`: Integer. Number of bootstrap samples used by SAFE (\eqn{10^6} by default). Larger values improve precision but increase run time. Used only when \code{SAFE = TRUE}.
- `parallelize`: Logical. If \code{TRUE} (default), SAFE calculations are parallelized over observations via \code{parallel::mclapply}, using \code{detectCores() - 1} workers. Set to \code{FALSE} to run serially. Note: \code{mclapply} uses forking (Unix/macOS); on Windows this may fall back to single-core behavior.
- `verbose`: Logical. If \code{TRUE} (default), prints the selected formulas, special warnings attached to the effect type (e.g., handling of zeros or negatives), and SAFE progress messages.
- `SAFE_distribution`: Character or \code{NULL}. Simulation family to use for SAFE. Must match a \code{sim_family} value in the formulas table. If \code{NULL} and the chosen \code{effect_type} has a default simulation family (\code{default_safe_family == "yes"}), that default is used. If no default is defined, the function does not alter the filtered formulas.
- `sigma_matrix`: List or \code{NULL}. Optional list of per-observation variance–covariance matrices used by SAFE (length equal to the number of rows implied by \code{...}). Each element should be a symmetric, positive definite matrix conformable with the simulated parameter vector required by the chosen effect formula. If \code{NULL}, internal defaults are used when available.
...
To understand how well our approximation is performing for sampling variance, we typically calculate *relative bias* by calculating bias as above, but dividing by the estimand and multiplying by 100.
...
} ### I DON"T THINK THIS IS CORRECT FOR RELATIVE BIAS AS I THINK YOU NEED TO TAKE THE SD OF THE ESTIMATES, NOT THE MEAN. I.E., RELATIVE BIAS OF VARIANCE ESTIMATES IS (SD(estimates) - TRUE VARIANCE) / TRUE VARIANCE * 100. PLEASE CHECK.  bias_sk_sv = ((mean(sk_sv) (mean of the sampling variance formula) - sd(sk)^2 (sd of the estimates)) / sd(sk)^2)*100, 
...
**Need to add explanation of 4-normal versus 2-normal/2-Wishart**
...
**Odd how different these SAFE results are compared to the others**
...
- Should summarize when SAFE is as good or better than plugin methods. Maybe a table?
