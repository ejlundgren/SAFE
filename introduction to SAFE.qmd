...
lnOR and lnRR, the dominant binomial count-based effect sizes, fail when the number of counts equals 0. For that reason, to make these effect sizes calculable, we use continuity corrections. 

The traditional continuity correction for lnOR is to add 0.5 to every cell (e.g., `a`, `b`, `c`, and `d`) for rows with a 0 in any cell. For lnRR (which only considers `a`, `c`, and `n1` and `n2`), the traditional correction is to add 0.5 to `a` or `c` and add 1 to `n1` or `n2`. 

Below, we conduct Monte Carlo simulations (@sec-continuity-monte-carlo) to compare this continuity correction with adding 1 to cells instead of 0.5.
...
Now let's evaluate how different values of $r$ affect bias and relative bias of SAFE and plugin estimators. We'll include an $r$ of 0, as well as 0.5 and 0.8 and we'll run each scenario 1e5 times. Note that the SAFE bootstrapping itself is happening in the `eff_size()` function. The simulated data in the `foreach` loop below is the Monte Carlo simulation (and is a draw of the raw data, not plausible means).

We'll do this for lnRoM and SMD.
...
bias <- ggplot(data = point.bias.long,
               aes(x = sample_n, y = bias, color = estimator))+
  geom_hline(yintercept = 0)+
  geom_path(linewidth = 1)+
  geom_point(size = 2)+
...
# Plot
rel_bias <- ggplot(data = var.bias.long, 
                   aes(x = sample_n, y = value, color = Estimator))+
  geom_hline(yintercept = 0)+
  geom_path(linewidth = 1)+
  geom_point(size = 2)+
...
# Continuity correction MC {#sec-continuity-monte-carlo}
