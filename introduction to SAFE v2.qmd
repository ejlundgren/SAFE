...
It may seem that we could just take the mean of this transformed cloud of hyperparameters to obtain an unbiased estimate of our effect size. However, due to Jensen's inequality, we cannot. Jensen's inequality states that applying any function (such as `1/x`) to the mean of a distribution does not equal taking the mean of that same function applied to the  distribution (@eq-inequality).
...
Where, $\bar{\theta}^*$ is the average of the bootstrapped estimates and $\hat{\theta}_{\text{truth}}$ is the true value. However, we do not know what the 'true' value is, given that our mean is a sample mean and the underlying population mean is unknowable. We thus use the plug-in estimate as surrogate for 'truth' (@eq-bias).
...
Bias corrected estimates just subtract this bias to correct the point estimate. In other words, the adjustment needed depends on the magnitude of difference and sign of the bias. We can substitute the bias into the following formula for bias-corrected estimate (@eq-bc):
...
*THESE RESULTS ARE NOW WAYYY OFF COMPARED TO THE FULL SIMULATIONS. THERE ARE SOME CRAZY HIGH VALUES in SAFE_VI THAT ARE CAUSING IT)
```{r, calc_rel_bias}
#| label: calc_rel_bias
#| eval: true
#| attr-source: "style='font-size: 0.8em;'"
...
range(res$vi_safe)
#' [CRAZY HIGH VALUES]
range(round(res[vi_safe > 10, ]$vi_safe))
#' [HOW DID THIS HAPPEN???]
...
We've conducted full Monte Carlo simulations for a variety of effect sizes @Monte-Carlo-results. These are below. However, first, we'll show how an analyst might use SAFE to calculate custom effect sizes and sampling variances.

## Creating custom effect sizes {#custom-effect-sizes}

The greatest draw of the SAFE method is its applicability to a variety of custom effect sizes that researchers may want to use for particular meta-analyses. After all, in order to conduct a meta-analysis one requires a measure of sampling variance. Deriving this sampling variance is non-trivial for many researchers as it requires using the Delta method and is difficult to confirm or validate. Let's try using SAFE for the temperature coefficient Q~10~.
